...
const EarningsEstimator = () => {
  ...
  const [animatedEarnings, setAnimatedEarnings] = useState<number>(hoursWorked * ratePerHour);
  const [selectedDistrict, setSelectedDistrict] = useState<District>(districts[0]);
  const [center, setCenter] = useState<LatLngExpression>([selectedDistrict.lat, selectedDistrict.lng]);
  ...
  return (
    <div className="flex flex-col mb-8 lg:flex-row justify-between items-center lg:items-start max-w-6xl mx-auto p-8 lg:py-12 space-y-8 lg:space-y-0 lg:space-x-12 pt-20">
      ...
      <div className="flex-1 space-y-4">
        ...
        <p className="text-2xl lg:text-3xl font-semibold">You could earn</p>
        <div className="text-5xl flex gap-x-3 lg:text-7xl font-bold">
          ₹
          <AnimatedNumbers includeComma animateToNumber={animatedEarnings} fontStyle={{ fontSize: 70 }} />
        </div>
        ...
        <div className="space-y-2">
          <div className="relative">
            <input
              ...
              className="w-full h-2 bg-blue-300 rounded-lg appearance-none cursor-pointer range-slider"
              style={{
          ...
          <p className="text-sm text-gray-600">Adjust hours worked per day: {hoursWorked} hours</p>
        </div>
        {/* District Selection */}
        <div className="space-y-2">
          <label className="text-sm text-gray-500">Select a District in Odisha:</label>
          <select
        ...
        <p className="text-sm text-gray-500 underline cursor-pointer">Learn how we estimate your earnings</p>
      </div>
      {/* Right Section: Map Display */}
      <div className="relative flex-1 w-full z-10 lg:w-1/2 h-[500px] bg-gray-200 rounded-lg">
        {typeof window !== 'undefined' && (
          <MapContainer center={center} zoom={10} scrollWheelZoom={false} className="h-full z-10 w-full rounded-lg">
            <TileLayer
