...
const trainingCenters: TrainingCenter[] = [
  {
    ...
    logo: "https://res.cloudinary.com/dgtc2fvgu/image/upload/v1726530978/scgj_n7hvdt.gif",
    description: "Located in Bhubaneswar, offering comprehensive green job training.",
  },
  {
    name: "Green Skills Academy - Cuttack",
    logo: "https://res.cloudinary.com/dgtc2fvgu/image/upload/v1726530978/scgj_n7hvdt.gif",
    description: "Located in Cuttack, providing hands-on training for green industries.",
  },
...
const Certification: React.FC = () => {
  ...
  const [selectedOption, setSelectedOption] = useState<'upload' | 'joinAcademy' | null>(null);
  const [selectedOrg, setSelectedOrg] = useState<string | null>(null);
  const [certificatePreview, setCertificatePreview] = useState<string | null>(null);

  const [selectedTrainingCenter, setSelectedTrainingCenter] = useState<string | null>(null);
  ...
  const referenceVideos = [
    {
      ...
      url: "https://www.youtube.com/embed/RsQWsFXbMLA",
    },
    {
  ...
  const handleAcademySubmit = () => {
    ...
    if (selectedTrainingCenter && selectedTimeSlot && address) {
      alert(`Training Center: ${selectedTrainingCenter}\nTime Slot: ${selectedTimeSlot}\nAddress: ${address}`);
        ...
        <div className="flex flex-col items-center justify-center bg-transparent p-8 lg:p-16">
          {isCertified ? (
            ...
            <div className="w-full">
              ...
              <div className="flex flex-col lg:flex-row items-center justify-between bg-gray-800 p-6 rounded-lg shadow-lg mb-8">
                {/* Certificate Image */}
                <div className="lg:w-1/2 w-full flex justify-center mb-4 lg:mb-0">
                  {certificatePreview ? (
                    ...
                    <div className="w-48 h-48 bg-gray-300 flex items-center justify-center rounded-lg">
                      <span className="text-gray-700">No Certificate Uploaded</span>
                    </div>
                  )}
                </div>
                ...
                <div className="lg:w-1/2 w-full flex flex-col items-center lg:items-start">
                  {selectedOrg ? (
                    <>
                      <img
                        src={organizations.find(org => org.name === selectedOrg)?.logo || ""}
                        alt={`${selectedOrg} logo`}
                        className="w-24 h-24 mb-4 object-contain"
                      />
                      <h2 className="text-2xl font-semibold text-white">{selectedOrg}</h2>
                      <p className="text-gray-300 mt-2">
                        {organizations.find(org => org.name === selectedOrg)?.name}
                      </p>
                    </>
                  ) : (
                    <>
                      ...
                      <p className="text-gray-300 mt-2">National Skill Development Corporation</p>
                    </>
                  )}
                </div>
              </div>
              ...
              {!selectedOption && (
                <div className="flex flex-col lg:flex-row justify-center items-center bg-gray-800 p-6 rounded-lg shadow-lg mb-8 space-y-4 lg:space-y-0 lg:space-x-8">
                  ...
                  >
                    ...
                    <span className="text-lg font-semibold text-gray-800">Upload Certificate</span>
                  </div>
                  ...
                  >
                    ...
                    <span className="text-lg font-semibold text-gray-800">Join Green Jobs Academy</span>
                  </div>
                </div>
              )}
                      ...
                      {!certificatePreview ? (
                        ...
                        <div className="relative flex justify-center items-center">
                          <img
                            ...
                            className="w-[300px] h-auto object-cover mt-4 rounded-lg"
                          />
                          <button
                            type="button"
                        ...
                        {trainingCenters.map((center) => (
                          <div
                            ...
                            className={`cursor-pointer flex flex-col items-center bg-white p-4 rounded-lg shadow-md hover:bg-green-100 transition transform hover:scale-105 ${
                              selectedTrainingCenter === center.name ? "border-2 border-green-500" : ""
                            }`}
                            onClick={() => handleTrainingCenterSelection(center.name)}
                          >
                            <img
                              src={center.logo}
